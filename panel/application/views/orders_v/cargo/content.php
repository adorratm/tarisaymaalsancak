...
<form id="updateOrderCargo" onsubmit="return false" method="post" enctype="multipart/form-data">
    <div class="mb-3">
                        ...
                        <div class="table-responsive">
                                ...
                                <table class="table table-bordered table-striped table-hover w-100">
                                    ...
                                    <tbody>
                                        <?php foreach ($order_data["cart"] as $cart_key => $cart_value) : ?>
                                            ...
                                            $select = "p.id,p.title,p.url,pi.url img_url,IFNULL(pvg.price,p.price) price,IFNULL(pvg.discount,p.discount) discount,p.vat vat,p.vatRate vatRate,IFNULL(pvg.stock,p.stock) stock,IFNULL(pvg.stockStatus,p.stockStatus) stockStatus,p.isActive,p.isDiscount isDiscount,p.sharedAt,CAST(JIFNULL(pvg.price,p.price) AS FLOAT) AS newPrice";
                                            $distinct = true;
                            ...
                            <table class="table table-bordered table-striped  w-100">
                                <thead class="table-dark">
                                    <tr>
                                        <th class="text-center align-middle">KARGO BİLGİLERİ</th>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <select name="shippingCompany" id="shippingCompany" class="form-control" required>
                                                <option value="MNG" <?= ($item->shippingCompany == "MNG" ? "selected" : "") ?>>MNG KARGO</option>
                                                <option value="ARAS" <?= ($item->shippingCompany == "ARAS" ? "selected" : "") ?>>ARAS KARGO</option>
                                                <option value="SÜRAT" <?= ($item->shippingCompany == "SÜRAT" ? "selected" : "") ?>>SÜRAT KARGO</option>
                                                <option value="YURTİÇİ" <?= ($item->shippingCompany == "YURTİÇİ" ? "selected" : "") ?>>YURTİÇİ KARGO</option>
                                            </select>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>
                                            Kargo Takip Kodu (Zorunludur.)
                                        </td>
                                        <td>
                                            <input type="text" class="form-control" value="<?= (!empty($item->shippingCode) ? $item->shippingCode : "") ?>" name="shippingCode" required>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
        ...
        <button data-url="<?= base_url("orders/update_cargo/$item->id"); ?>" class="btn btn-sm btn-outline-primary rounded-0 btnUpdateCargo">Güncelle</button>
