	...
	<div class="row">
		<div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
			<h4 class="mb-3">
				...
				<a href="<?= base_url("products/priceform") ?>" class="btn btn-sm d-none btn-outline-info rounded-0 float-right"> <i class="fa fa-dropbox"></i> Toplu Ürün Güncelleme</a>
			</h4>
		</div>
		<div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
			<form id="filter_form" onsubmit="return false">
				<div class="d-flex flex-wrap">
					...
					<label for="price_button" class="mx-1 toggleLabel d-none">
						<button class="btn btn-sm btn-outline-primary rounded-0 " data-url="<?= base_url("products/priceForm") ?>" id="price_button" data-toggle="tooltip" data-placement="top" data-title="Seçili Ürünlerin Fiyatını Güncelle"><i class="fa fa-hand-holding-usd"></i></button>
					</label>
...
<script>
	...
	$(document).ready(function() {
		...
		$(document).on("click", "#price_button", function(e) {
			...
			idArray.sort();
			$('#productModal').iziModal('destroy');
			createModal("#productModal", "Seçili Ürünlerin Fiyatlarını Güncelle", "Seçili Ürünlerin Fiyatlarını Güncelle", 600, true, "20px", 0, "#e20e17", "#fff", 1040, function() {
				$.post(url, {
					"product_ids": idArray
				}, function(response) {
					$("#productModal .iziModal-content").html(response);
			...
			openModal("#productModal");
			$("#productModal").iziModal("setFullscreen", true);
		});
		$(document).on("click", "#btnUpdateRateAsc", function(e) {
			e.preventDefault();
			e.stopImmediatePropagation();
			let url = $(this).data("url");
			let formData = new FormData(document.getElementById("priceRateForm"));
		...
		$(document).on("click", "#btnUpdateRateDesc", function(e) {
			e.preventDefault();
			e.stopImmediatePropagation();
			let url = $(this).data("url");
			let formData = new FormData(document.getElementById("priceRateForm"));
			formData.append("ascending", false);
			createAjax(url, formData, function() {
				closeModal("#productModal");
				$("#productModal").iziModal("setFullscreen", false);
				reloadTable("productTable");
