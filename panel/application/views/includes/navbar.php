...
<nav class="navbar navbar-expand-xl navbar-dark fixed-top hk-navbar">
    ...
    <ul class="navbar-nav hk-navbar-content">
        ...
        */ ?>
        <?php $languages = $this->general_model->get_all("settings", null, "rank ASC", ["isActive" => 1]) ?>
        <?php $activeLang = $this->session->userdata("activeLang") ?>
        <?php if (empty($activeLang)) : ?>
            <?php
            $activeLang = "tr";
            $this->session->set_userdata("activeLang", "tr");
            ?>
        <?php endif ?>
        <li class="nav-item dropdown dropdown-authentication">
            <a class="nav-link dropdown-toggle no-caret" href="javascript:void(0)" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-globe"></i> <small class="d-none d-sm-inline"><?= strto("lower|upper", $activeLang); ?> <i class="ml-1 fa fa-chevron-down"></i></small></a>
            <div class="dropdown-menu rounded-0 dropdown-menu-right" data-dropdown-in="flipInX" data-dropdown-out="flipOutX">
                <?php foreach ($languages as $lKey => $lValue) : ?>
                    <a class="dropdown-item" href="<?= base_url("dashboard/switchlanguage/{$lValue->lang}") ?>"><i class="dropdown-icon fa fa-language"></i><span><?= strto("lower|upper", $lValue->lang) ?></span></a>
                <?php endforeach ?>
            </div>
        </li>
        <li class="nav-item dropdown dropdown-authentication">
