...
class News extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $viewData->items = $this->news_model->get_all([], "rank ASC");
        $this->load->view("{$this->viewFolder}/{$viewData->subViewFolder}/index", $viewData);
    }
    public function datatable()
    {
        $items = $this->news_model->getRows([], $_POST);
        $data = [];
        ...
        if (checkEmpty($data)["error"]) :
            ...
            if (!empty($_FILES)) :
                ...
                if ($image["success"]) :
                    ...
                    echo json_encode(["success" => false, "title" => "Başarısız!", "message" => "Haber Kaydı Yapılırken Hata Oluştu. Haber Görseli Seçtiğinizden Emin Olup Tekrar Deneyin."]);
                    die();
                endif;
            else :
                echo json_encode(["success" => false, "title" => "Başarısız!", "message" => "Haber Kaydı Yapılırken Hata Oluştu. Haber Görseli Seçtiğinizden Emin Olup Tekrar Deneyin."]);
    ...
    public function update_form($id)
    {
        ...
        $viewData->categories = $this->news_category_model->get_all();
        $viewData->item = $this->news_model->get(["id" => $id]);
        $viewData->settings = $this->general_model->get_all("settings", null, null, ["isActive" => 1]);
        $this->load->view("{$this->viewFolder}/{$viewData->subViewFolder}/content", $viewData);
    }
    public function update($id)
    {
        ...
        if (checkEmpty($data)["error"]) :
            ...
            $data["img_url"] = $news->img_url;
            if (!empty($_FILES["img_url"]["name"])) :
                $image = upload_picture("img_url", "uploads/$this->viewFolder");
                if ($image["success"]) :
                    $data["img_url"] = $image["file_name"];
                    if (!empty($news->img_url)) :
                        if (!is_dir(FCPATH . "uploads/{$this->viewFolder}/{$news->img_url}") && file_exists(FCPATH . "uploads/{$this->viewFolder}/{$news->img_url}")) :
                            unlink(FCPATH . "uploads/{$this->viewFolder}/{$news->img_url}");
                        endif;
                    endif;
                else :
                    echo json_encode(["success" => false, "title" => "Başarısız!", "message" => "Haber Güncelleştirilirken Hata Oluştu. Haber Görseli Seçtiğinizden Emin Olup Tekrar Deneyin."]);
                    die();
                endif;
            endif;
            $data["seo_url"] = seo($data["title"]);
    ...
    public function rankSetter()
    {
        ...
        if (!empty($rows)) :
            foreach ($rows as $row) :
                $this->news_model->update(["id" => $row["id"]], ["rank" => $row["position"]]);
            endforeach;
        endif;
    }
