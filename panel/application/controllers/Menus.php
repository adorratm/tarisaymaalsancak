...
class Menus extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $items = $this->menu_model->get_all(["lang" => $this->session->userdata("activeLang")]);
    ...
    {
        $items = $this->menu_model->getRows(["lang" => $this->session->userdata("activeLang")], $_POST);
        $data = [];
        ...
        $output = [
            ...
            "recordsTotal" => $this->menu_model->rowCount(["lang" => $this->session->userdata("activeLang")]),
            "recordsFiltered" => $this->menu_model->countFiltered(["lang" => $this->session->userdata("activeLang")], (!empty($_POST) ? $_POST : [])),
            "data" => $data,
        ];
    ...
    {
        $viewData = new stdClass();
        $viewData->top_menus = $this->menu_model->get_all(["lang" => $this->session->userdata("activeLang")]);
        $viewData->pages = $this->page_model->get_all(["lang" => $this->session->userdata("activeLang")]);
        $viewData->viewFolder = $this->viewFolder;
    ...
    public function save()
    {
        ...
        if (checkEmpty($data)["error"] && (checkEmpty($data)["key"] != "url" && checkEmpty($data)["key"] != "page_id" && checkEmpty($data)["key"] != "top_id")) :
            ...
            $getRank = $this->menu_model->rowCount(["lang" => $this->session->userdata("activeLang")]);
            $data["isActive"] = 1;
            $data["rank"] = $getRank + 1;
    ...
    public function update_form($id)
    {
        ...
        $viewData->item = $this->menu_model->get(["id" => $id,"lang" => $this->session->userdata("activeLang")]);
        $viewData->settings = $this->general_model->get_all("settings", null, null, ["isActive" => 1,"lang" => $this->session->userdata("activeLang")]);
        $this->load->view("{$viewData->viewFolder}/{$viewData->subViewFolder}/content", $viewData);
    }
    ...
    {
        $menu = $this->menu_model->get(["id" => $id,"lang" => $this->session->userdata("activeLang")]);
        if (!empty($menu)) :
