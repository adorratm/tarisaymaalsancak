...
class Emailsettings extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $viewData->items = $this->email_settings_model->get_all(["lang" => $this->session->userdata("activeLang")]);
        $this->load->view("{$this->viewFolder}/{$viewData->subViewFolder}/index", $viewData);
    }
    public function datatable()
    {
        $items = $this->email_settings_model->getRows(["lang" => $this->session->userdata("activeLang")], $_POST);
        $data = [];
        ...
        $output = [
            ...
            "recordsTotal" => $this->email_settings_model->rowCount(["lang" => $this->session->userdata("activeLang")]),
            "recordsFiltered" => $this->email_settings_model->countFiltered(["lang" => $this->session->userdata("activeLang")], (!empty($_POST) ? $_POST : [])),
            "data" => $data,
        ];
    ...
    public function save()
    {
        ...
        if (checkEmpty($data)["error"]) :
            ...
            $getRank = $this->email_settings_model->rowCount(["lang" => $this->session->userdata("activeLang")]);
    ...
    public function update_form($id)
    {
        ...
        $viewData->item = $this->email_settings_model->get(["id" => $id,"lang" => $this->session->userdata("activeLang")]);
        $viewData->settings = $this->general_model->get_all("settings", null, null, ["isActive" => 1,"lang" => $this->session->userdata("activeLang")]);
        $this->load->view("{$this->viewFolder}/{$viewData->subViewFolder}/content", $viewData);
    }
    ...
    {
        $emailsettings = $this->email_settings_model->get(["id" => $id,"lang" => $this->session->userdata("activeLang")]);
