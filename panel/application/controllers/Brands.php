...
class Brands extends MY_Controller
{
    ...
    public function update_form($id)
    {
        ...
        $viewData->item = $this->brand_model->get(["id" => $id,"lang" => $this->session->userdata("activeLang")]);
        $viewData->settings = $this->general_model->get_all("settings", null, null, ["isActive" => 1,"lang" => $this->session->userdata("activeLang")]);
        $this->load->view("{$this->viewFolder}/{$viewData->subViewFolder}/content", $viewData);
    }
    public function update($id)
    {
        ...
        if (checkEmpty($data)["error"]) :
            ...
            $brand = $this->brand_model->get(["id" => $id,"lang" => $this->session->userdata("activeLang")]);
            ...
            $update = $this->brand_model->update(["id" => $id,"lang" => $this->session->userdata("activeLang")], $data);
            if ($update) :
