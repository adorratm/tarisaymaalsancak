...
class News_categories extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $viewData->items = $this->news_category_model->get_all();
        $this->load->view("{$viewData->viewFolder}/{$viewData->subViewFolder}/index", $viewData);
    }
    public function datatable()
    {
        $items = $this->news_category_model->getRows([], $_POST);
        $data = [];
    ...
    public function update_form($id)
    {
        ...
        $viewData->item = $this->news_category_model->get(["id" => $id]);
        $viewData->settings = $this->general_model->get_all("settings", null, null, ["isActive" => 1]);
        $this->load->view("{$viewData->viewFolder}/{$viewData->subViewFolder}/content", $viewData);
    }
    public function update($id)
    {
        ...
        if (checkEmpty($data)["error"]) :
            ...
            if (!empty($news_category->img_url)) :
                $data["img_url"] = $news_category->img_url;
            endif;
            if (!empty($_FILES["img_url"]["name"])) :
                ...
                if ($image["success"]) :
                    ...
                    if (!empty($news_category->img_url)) :
                        if (!is_dir(FCPATH . "uploads/{$this->viewFolder}/{$news_category->img_url}") && file_exists(FCPATH . "uploads/{$this->viewFolder}/{$news_category->img_url}")) :
                            unlink(FCPATH . "uploads/{$this->viewFolder}/{$news_category->img_url}");
                        endif;
                    endif;
                else :
                    ...
                    foreach ($news as $key => $value) :
                        if ($value->category_id == $id) :
                            ...
                            if (!empty($value->img_url)) :
                                if (!is_dir(FCPATH . "uploads/news_v/{$value->img_url}") && file_exists(FCPATH . "uploads/news_v/{$value->img_url}")) :
                                    unlink(FCPATH . "uploads/news_v/{$value->img_url}");
                                endif;
                            endif;
                        endif;
                    endforeach;
