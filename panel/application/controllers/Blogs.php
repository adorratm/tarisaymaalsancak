...
class Blogs extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $viewData->items = $this->blog_model->get_all(["lang" => $this->session->userdata("activeLang")], "rank ASC");
        $this->load->view("{$this->viewFolder}/{$viewData->subViewFolder}/index", $viewData);
    }
    public function datatable()
    {
        $items = $this->blog_model->getRows(["lang" => $this->session->userdata("activeLang")], $_POST);
        $data = [];
        ...
        $output = [
            ...
            "recordsTotal" => $this->blog_model->rowCount(["lang" => $this->session->userdata("activeLang")]),
            "recordsFiltered" => $this->blog_model->countFiltered(["lang" => $this->session->userdata("activeLang")], (!empty($_POST) ? $_POST : [])),
            "data" => $data,
        ];
    ...
    public function save()
    {
        ...
        if (checkEmpty($data)["error"]) :
            ...
            $getRank = $this->blog_model->rowCount(["lang" => $this->session->userdata("activeLang")]);
            if (!empty($_FILES)) :
    ...
    public function update_form($id)
    {
        ...
        $viewData->categories = $this->blog_category_model->get_all(["lang" => $this->session->userdata("activeLang")]);
        $viewData->item = $this->blog_model->get(["id" => $id,"lang" => $this->session->userdata("activeLang")]);
        $viewData->settings = $this->general_model->get_all("settings", null, null, ["isActive" => 1,"lang" => $this->session->userdata("activeLang")]);
        $this->load->view("{$this->viewFolder}/{$viewData->subViewFolder}/content", $viewData);
    }
    public function update($id)
    {
        ...
        if (checkEmpty($data)["error"]) :
            ...
            $blog = $this->blog_model->get(["id" => $id,"lang" => $this->session->userdata("activeLang")]);
