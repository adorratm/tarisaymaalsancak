...
class Pages extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $viewData->items = $this->page_model->get_all(["lang" => $this->session->userdata("activeLang")], "rank ASC");
        $this->load->view("{$this->viewFolder}/{$viewData->subViewFolder}/index", $viewData);
    }
    public function datatable()
    {
        $items = $this->page_model->getRows(["lang" => $this->session->userdata("activeLang")], $_POST);
        $data = [];
        ...
        $output = [
            ...
            "recordsTotal" => $this->page_model->rowCount(["lang" => $this->session->userdata("activeLang")]),
            "recordsFiltered" => $this->page_model->countFiltered(["lang" => $this->session->userdata("activeLang")], (!empty($_POST) ? $_POST : [])),
            "data" => $data,
        ];
    ...
    public function save()
    {
        ...
        if (checkEmpty($data)["error"] && checkEmpty($data)["key"] !== "content") :
            ...
            $getRank = $this->page_model->rowCount(["lang" => $this->session->userdata("activeLang")]);
            if (!empty($_FILES)) :
    ...
    public function update_form($id)
    {
        ...
        $viewData->item = $this->page_model->get(["id" => $id,"lang" => $this->session->userdata("activeLang")]);
        $viewData->settings = $this->general_model->get_all("settings", null, null, ["isActive" => 1,"lang" => $this->session->userdata("activeLang")]);
        $this->load->view("{$this->viewFolder}/{$viewData->subViewFolder}/content", $viewData);
    }
    ...
    {
        $data = rClean($this->input->post());
        if (checkEmpty($data)["error"] && checkEmpty($data)["key"] !== "img_url" && checkEmpty($data)["key"] !== "content") :
            $key = checkEmpty($data)["key"];
            ...
            $page = $this->page_model->get(["id" => $id,"lang" => $this->session->userdata("activeLang")]);
