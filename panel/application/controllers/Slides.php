...
class Slides extends MY_Controller
{
    ...
    {
        $items = $this->slide_model->getRows(["lang" => $this->session->userdata("activeLang")], $_POST);
        $data = [];
        ...
        $output = [
            ...
            "recordsTotal" => $this->slide_model->rowCount(["lang" => $this->session->userdata("activeLang")]),
            "recordsFiltered" => $this->slide_model->countFiltered(["lang" => $this->session->userdata("activeLang")], (!empty($_POST) ? $_POST : [])),
            "data" => $data,
        ];
    ...
    public function index()
    {
        ...
        $viewData->items = $this->slide_model->get_all(["lang" => $this->session->userdata("activeLang")], "rank ASC");
    ...
    public function save()
    {
        ...
        $getRank = $this->slide_model->rowCount(["lang" => $this->session->userdata("activeLang")]);
    ...
    public function update_form($id)
    {
        ...
        $viewData->item = $this->slide_model->get(["id" => $id, "lang" => $this->session->userdata("activeLang")]);
        $viewData->pages = $this->general_model->get_all("pages", null, "rank ASC", ["isActive" => 1, "lang" => $this->session->userdata("activeLang")]);;
        $viewData->categories = $this->general_model->get_all("product_categories", null, "rank ASC", ["isActive" => 1, "lang" => $this->session->userdata("activeLang")]);
        $viewData->products = $this->general_model->get_all("products p", "p.id,p.title", "p.rank ASC", ["p.isActive" => 1, "pi.isCover" => 1, "p.lang" => $this->session->userdata("activeLang")], [], ["products_w_categories pwc" => ["p.id = pwc.product_id", "left"], "product_categories pc" => ["pwc.category_id = pc.id", "left"], "product_variation_groups pvg" => ["p.id = pvg.product_id", "left"], "product_images pi" => ["pi.product_id = p.id", "left"]], [], [], true, ["p.id", "pwc.product_id", "pvg.id"]);
        $viewData->settings = $this->general_model->get_all("settings", null, null, ["isActive" => 1, "lang" => $this->session->userdata("activeLang")]);
    ...
    {
        $data = rClean($this->input->post());
        $slide = $this->slide_model->get(["id" => $id, "lang" => $this->session->userdata("activeLang")]);
        if (!empty($data["allowButton"]) && $data["allowButton"] == "on" && (empty($data["button_caption"]))) :
            echo json_encode(["success" => false, "title" => "Başarısız!", "message" => "Slayt Kaydı Yapılırken Hata Oluştu. Buton Başlık ve URL Bilgisini Doldurduğunuzdan Emin Olup Tekrar Deneyin."]);
            die();
        else :
