...
class Galleries extends MY_Controller
{
    ...
    {
        $items = $this->gallery_model->getRows(["lang" => $this->session->userdata("activeLang")], $_POST);
        $data = [];
        ...
        $output = [
            ...
            "recordsTotal" => $this->gallery_model->rowCount(["lang" => $this->session->userdata("activeLang")]),
            "recordsFiltered" => $this->gallery_model->countFiltered(["lang" => $this->session->userdata("activeLang")], (!empty($_POST) ? $_POST : [])),
            "data" => $data,
        ];
    ...
    public function save()
    {
        ...
        if (checkEmpty($data)["error"]) :
            ...
            $getRank = $this->gallery_model->rowCount(["lang" => $this->session->userdata("activeLang")]);
    ...
    public function update_form($id)
    {
        ...
        $viewData->item = $this->gallery_model->get(["id" => $id,"lang" => $this->session->userdata("activeLang")]);
        $viewData->settings = $this->general_model->get_all("settings", null, null, ["isActive" => 1,"lang" => $this->session->userdata("activeLang")]);
        $this->load->view("{$viewData->viewFolder}/{$viewData->subViewFolder}/content", $viewData);
    }
    public function update($id, $gallery_type)
    {
        ...
        if (checkEmpty($data)["error"]) :
            ...
            $gallery = $this->gallery_model->get(["id" => $id,"lang" => $this->session->userdata("activeLang")]);
            $oldFolderName = null;
            $folder_name = null;
            if (!empty($gallery)) :
    ...
    {
        $gallery = $this->gallery_model->get(["id" => $gallery_id,"lang" => $this->session->userdata("activeLang")]);
        $modelName = ($gallery_type == "images" ? "image_model" : ($gallery_type == "files" ? "file_model" : ($gallery_type == "videos" ? "video_model" : "video_url_model")));
        $items = $this->$modelName->getRows(["gallery_id" => $gallery_id,"lang" => $this->session->userdata("activeLang")], $_POST);
        $data = [];
        $i = (!empty($_POST['start']) ? $_POST['start'] : 0);
        if (!empty($items)) :
            foreach ($items as $item) :
        ...
        $output = [
            "draw" => (!empty($_POST['draw']) ? $_POST['draw'] : 0),
            "recordsTotal" => $this->$modelName->rowCount(["gallery_id" => $gallery_id,"lang" => $this->session->userdata("activeLang")]),
            "recordsFiltered" => $this->$modelName->countFiltered(["gallery_id" => $gallery_id,"lang" => $this->session->userdata("activeLang")], (!empty($_POST) ? $_POST : [])),
            "data" => $data,
        ];
        // Output to JSON format
        echo json_encode($output);
    ...
    public function upload_form($id)
    {
        ...
        if ($viewData->item->gallery_type == "images") :
            $viewData->items = $this->image_model->get_all(["gallery_id" => $id,"lang" => $this->session->userdata("activeLang")], "rank ASC");
        elseif ($viewData->item->gallery_type == "files") :
            $viewData->items = $this->file_model->get_all(["gallery_id" => $id,"lang" => $this->session->userdata("activeLang")], "rank ASC");
        elseif ($viewData->item->gallery_type == "videos") :
            $viewData->items = $this->video_model->get_all(["gallery_id" => $id,"lang" => $this->session->userdata("activeLang")], "rank ASC");
        else :
            $viewData->items = $this->video_url_model->get_all(["gallery_id" => $id,"lang" => $this->session->userdata("activeLang")], "rank ASC");
        endif;
        $this->load->view("{$viewData->viewFolder}/{$viewData->subViewFolder}/index", $viewData);
    }
    ...
    {
        $viewData = new stdClass();
        $viewData->gallery = $this->gallery_model->get(['id' => $gallery_id,"lang" => $this->session->userdata("activeLang")]);
        if ($viewData->gallery->gallery_type == "images") :
            $viewData->item = $this->image_model->get(["id" => $id, "gallery_id" => $viewData->gallery->id,"lang" => $this->session->userdata("activeLang")]);
        elseif ($viewData->gallery->gallery_type == "files") :
            $viewData->item = $this->file_model->get(["id" => $id, "gallery_id" => $viewData->gallery->id,"lang" => $this->session->userdata("activeLang")]);
        elseif ($viewData->gallery->gallery_type == "videos") :
            $viewData->item = $this->video_model->get(["id" => $id, "gallery_id" => $viewData->gallery->id,"lang" => $this->session->userdata("activeLang")]);
        else :
            $viewData->item = $this->video_url_model->get(["id" => $id, "gallery_id" => $viewData->gallery->id,"lang" => $this->session->userdata("activeLang")]);
        endif;
        $viewData->viewFolder = $this->viewFolder;
    ...
    public function file_upload($gallery_id, $gallery_type, $lang)
    {
        ...
        if ($gallery_type != "video_urls") :
            if ($gallery_type == "images") :
                ...
                if ($image["success"]) :
                    $getRank = $this->image_model->rowCount(["lang" => $this->session->userdata("activeLang")]);
            ...
            elseif ($gallery_type == "videos") :
                ...
                if ($image["success"]) :
                    $getRank = $this->video_model->rowCount(["lang" => $this->session->userdata("activeLang")]);
                ...
                if ($image["success"]) :
                    $getRank = $this->file_model->rowCount(["lang" => $this->session->userdata("activeLang")]);
                    $this->file_model->add(
                        [
                            "url"           => $image["file_name"],
                            "rank"          => $getRank + 1,
            ...
            if (checkEmpty($data)["error"] && checkEmpty($data)["key"] !== "url") :
                ...
                $getRank = $this->video_url_model->rowCount(["lang" => $this->session->userdata("activeLang")]);
    ...
    {
        $modelName = ($gallery_type == "images" ? "image_model" : ($gallery_type == "files" ? "file_model" : ($gallery_type == "videos" ? "video_model" : "video_url_model")));
        $gallery = $this->gallery_model->get(["id" => $parent_id,"lang" => $this->session->userdata("activeLang")]);
        $fileName = $this->$modelName->get(["id" => $id,"lang" => $this->session->userdata("activeLang")]);
