...
function checkWishlist($userWishlistArray = [], $product_id = null)
{
    return ((in_array($product_id, $userWishlistArray) ? "text-danger" : null));
}
...
function in_parentt($in_parent = null, $lang = null, $store_all_id = null)
{
    ...
    if (in_array($in_parent, $store_all_id)) :
        ...
        foreach ($result as $key => $value) :
            $html .= '<li>';
            $html .= '<a rel="dofollow" ' .
                ($t->uri->segment(3) == $value->seo_url ? "class='active link'" : "class='link'") . ' href="' . base_url(lang("routes_products") . "/{$value->seo_url}") . '" title="' . $value->title . '"><i class="fa fa-chevron-right"></i> ' . $value->title . '</a>';
...
function show_product_categories($checkedValues = null, $lang = 'tr')
{
    $t = &get_instance();
    // create array to store all menus ids
    $store_all_id = [];
    // get all parent menus ids by using isactive
    $id_result = $t->general_model->get_all("product_variation_categories", "top_id", "rank ASC", ["isActive" => 1]);
    // loop through all menus to save parent ids $store_all_id array
    if (!empty($id_result)) :
        foreach ($id_result as $menu_id) :
            array_push($store_all_id, $menu_id->top_id);
 ...
 * through all comments and retrieve it
 */
function product_in_parent($in_parent = null, $checkedValues = null, $lang = null, $store_all_id = null)
{
    $t = &get_instance();
    $variation_groups = $t->general_model->get_all("product_variation_groups", "category_id,variation_id", "rank ASC", ["isActive" => 1]);
    $checkedValues = !empty($_GET["selectedVariations"]) && is_array($_GET["selectedVariations"]) ? rClean($_GET["selectedVariations"]) : (!empty($_GET["selectedVariations"]) ? [clean($_GET["selectedVariations"])] : []);
    $availableCategories = [];
    $availableVariations = [];
    if (!empty($variation_groups)) :
        foreach ($variation_groups as $y => $z) :
            if (!empty($z->category_id)) :
                foreach (explode(",", $z->category_id) as $in => $cat) :
                    if (!in_array(intVal($cat), $availableCategories)) :
                        array_push($availableCategories, intVal($cat));
                    endif;
                endforeach;
            endif;
            if (!empty($z->variation_id)) :
                foreach (explode(",", $z->variation_id) as $in2 => $cat2) :
                    if (!in_array(intVal($cat2), $availableVariations)) :
                        array_push($availableVariations, intVal($cat2));
                    endif;
                endforeach;
            endif;
        endforeach;
    ...
    // build hierarchy  html structure based on ul li (parent-child) nodes
    if (in_array($in_parent, $store_all_id)) :
        $result = $t->general_model->get_all("product_variation_categories", "id,title", "rank ASC", ["top_id" => $in_parent, "isActive" => 1]);
        if (!empty($result)) :
            foreach ($result as $key => $value) :
                $variations = $t->general_model->get_all("product_variations", "id,title", "rank ASC", ["category_id" => $value->id, "isActive" => 1]);
                if (!empty($variations)) :
                    if (!empty($availableVariations)) :
                        $html .=  "<div class='filter-items'>";
                        $html .=  "<div class='filtr-cate-title'>";
                        $html .= "<h4>" . $value->title . "</h4>";
                        $html .=  "</div>";
                        $html .=  "<div class='other-item-body h-auto scrollstyle_4'>";
                        $html .= "<div class='brand-list'>";
                        foreach ($variations as $k => $v) :
                            if (in_array($v->id, $availableVariations)) :
                                $html .= "<div class='form-check mb-2'>";
                                $html .= "<input id='input-" . $v->id . "' type='checkbox' class='form-check-input' name='selectedVariations[]' " . (in_array($v->id, $checkedValues) ? "checked" : null) . " value='" . $v->id . "'>";
                                $html .= "<label for='input-" . $v->id . "'>" . $v->title . "</label>";
                                $html .= "</div>";
                            endif;
                        endforeach;
                        $html .= "</div>";
                ...
                $html .= product_in_parent($value->id, $checkedValues, $lang, $store_all_id);
            endforeach;
        endif;
    endif;
    return $html;
...
function in_parenttheader($in_parent = null, $lang = null, $store_all_id = null)
{
    ...
    // build hierarchy  html structure based on ul li (parent-child) nodes
    if (in_array($in_parent, $store_all_id)) :
        $result = $t->general_model->get_all("product_categories", "title,seo_url,id,top_id,icon", "rank ASC", ["top_id" => $in_parent, "isActive" => 1, "lang" => $lang]);
        $html .=  '<ul class="' . ($in_parent == 0 ? "category-by-cat" : "dropdown-menu") . '">';
        /*
        if ($in_parent == 0) :
            $html .= '<li class="nav-item d-lg-none"><form id="searchFormMobile" action="' . (!empty($t->uri->segment(2) && !is_numeric($t->uri->segment(2))) ? base_url(lang("routes_products")) : base_url(lang("routes_products"))) . '" method="GET" enctype="multipart/form-data">
            <input type="hidden" name="' . $t->security->get_csrf_token_name() . '" value="' . $t->security->get_csrf_hash() . '" />
            ...
            if (!empty($t->viewData->menuCategories)) :
                foreach ($t->viewData->menuCategories as $key => $value) :
                    if ($value->top_id == 0) :
                        $html .= '<option value="' . base_url(lang("routes_products") . "/" . $value->seo_url) . '">' . $value->title . '</option>';
                    endif;
                endforeach;
            endif;
            $html .= '</select>
                ...
                <button type="submit" class="btn btn-secondary bg-main-dark" aria-label="' . $t->viewData->settings->company_name . '"><i class="fa fa-search"></i></button>
            </div>
        </form><li>';
        endif;
        */
        foreach ($result as $key => $value) :
            ...
            $html .= '<a rel="dofollow" class="text-center ' . ($t->uri->segment(3) == $value->seo_url ? "active single-cat-item" : "single-cat-item") . '" href="' . base_url(lang("routes_products") . "/{$value->seo_url}") . '" title="' . $value->title . '">';
            if (!empty($value->icon) && $value->top_id == 0) :
                $html .= '<div class="icon"><img data-src="' . get_picture("product_categories_v", $value->icon) . '" class="lazyload img-fluid d-flex justify-content-center align-content-center align-self-center mx-auto" width="75" height="75"></div>';
            endif;
            $html .= '<div class="text">' . $value->title . '</div></a>';
