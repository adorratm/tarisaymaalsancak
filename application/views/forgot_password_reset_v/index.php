                ...
                <div class="default_tabs">
                    <nav>
                        <div class="nav nav-tabs tab_default  justify-content-center">
                            ...
                            <a class="nav-item nav-link active" href="<?= str_replace("index.php/" . $lang . "/", "", current_url()) ?>"><?= strto("lower|ucwords", lang("forgotPasswordReset")) ?></a>
                        </div>
                    </nav>
                </div>
                <div class="title129">
                    <h2><?= strto("lower|ucwords", lang("forgotPasswordReset")) ?></h2>
                </div>
            </div>
        </div>
    </div>
                        ...
                        <div class="form-dt">
                            <div class="form-inpts checout-address-step">
                                <form action="<?= base_url(lang("routes_forgot-password-reset")) ?>" method="POST" enctype="multipart/form-data">
                                    ...
                                    <input type="hidden" name="token" class="form-control" required placeholder="<?= lang("token") ?>*" value="<?= !empty($_GET["token"]) ? $_GET["token"] : null ?>">
                                    <div class="form-title">
                                        <h6><?= strto("lower|ucwords", lang("login")) ?></h6>
                                    </div>
                                    ...
                                    <button class="login-btn hover-btn h_50" type="submit"><?= strto("lower|ucwords", lang("submitForgotPasswordReset")); ?></button>
                                </form>
                            </div>
